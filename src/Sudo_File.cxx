    ...
    if (hfile) {
        ...
        if (size) {
            ...
            if (buf) {
                ...
                if (size == res) {
                    buf[size] = 0;
                    Process_Buffer(file,buf,size);
                    if (is_curr_box_valid_for_paint()) {
                        ok = true;
                    }
                } else {
                    sprtf("ERROR: File read failed! req %d, got %d!\n", (int)size, (int)res);
                }
                free(buf);
            }
        } else {
            sprtf("ERROR: File '%s' has no SIZE!\n",file);
        }
        fclose(hfile);
    }
    if (ok) {
...
bool Save_XML_File(HWND hWnd, char *pf, int flag)
{
    ...
    wtn = fwrite(xml_head,1,len,hfile);
    PABOX2 pb = get_curr_box();
    int row, col, val;
    char *ps = _s_buf;
    *ps = 0;
...
bool Test_Sudoku_Load(PABOX2 pb, char *pfile)
{
    ...
    if (hfile) {
        ...
        if (size) {
            char *buf = (char *)malloc(size+16);
            if (buf) {
                size_t res = fread(buf,1,size,hfile);
                if (size == res)
                {
                    buf[size] = 0;
                    if ((Test_Process_Buffer(pb,pfile,buf,size) == 0) &&
                        ( validate_box(pb) == 0 ) )
                    {
                        ok = true;
                    }
...
//////////////////////////////////////////////////////////////////////////////////
// TODO: Port this - Uses WINDOWS FindFirstFile
#ifdef WIN32 // windows only at present
