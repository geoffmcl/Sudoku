...
Timer::Timer()
{
#ifdef WIN32    // windows - use QueryPerformanceFrequency
    QueryPerformanceFrequency(&frequency);
    startCount.QuadPart = 0;
    endCount.QuadPart = 0;
#else   // !WIN32 - alterative timer
...
#ifdef WIN32    // windows - QueryPerformanceCounter
    QueryPerformanceCounter(&startCount);
#else // !WIN32 - use gettimeofday
    gettimeofday(&startCount, NULL);
#endif // WIN32 y/n - start timer
}
...
#ifdef WIN32 // windows - use QueryPerformanceCounter
    QueryPerformanceCounter(&endCount);
#else // !WIN32 - use gettimeofday
    gettimeofday(&endCount, NULL);
#endif // WIN32 y/n - timer stop
}
...
{
#ifdef WIN32 // windows QueryPerformanceCounte
    if(!stopped)
        QueryPerformanceCounter(&endCount);

    startTimeInMicroSec = startCount.QuadPart * (1000000.0 / frequency.QuadPart);
    endTimeInMicroSec = endCount.QuadPart * (1000000.0 / frequency.QuadPart);
#else   // !WIN32 - gettimeofday
    if(!stopped)
        gettimeofday(&endCount, NULL);

    startTimeInMicroSec = (startCount.tv_sec * 1000000.0) + startCount.tv_usec;
    endTimeInMicroSec = (endCount.tv_sec * 1000000.0) + endCount.tv_usec;
#endif  // WIN32 - Timer::getElapsedTimeInMicroSec
